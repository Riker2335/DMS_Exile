...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_boat_mission.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	case "moderate":
	{
		...
		_crate_weapons 		= (10 + (round (random 4)));
	...
	case "difficult":
	{
		...
		_crate_weapons 		= (12 + (round (random 5)));
...
_msgStart = ['#FFFF00',"Terrorists have managed to Crash their Vehicle. Kill the Terrorists and steal the BTR60!"];
...
if (_difficulty isEqualTo "hardcore") then
{
	...
	_vehicle = ["CUP_O_BTR60_TK",_pos getPos [30, random 360],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#0080ff',format ["Convicts have got their hands on the weapons, entry code for BTR60 is %1...",_pinCode]];
} else
{
	_vehicle = ["CUP_O_BTR60_TK",_pos getPos [30, random 360]] call DMS_fnc_SpawnNonPersistentVehicle;
...
_msgLOSE = ['#FF0000',"The Terrorists had enough time to prepare an attack, no guns for you..."];

// Define mission name (for map marker and logging)
_missionName = "Terrorists";
