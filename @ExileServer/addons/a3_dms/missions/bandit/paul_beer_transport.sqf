...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_beer_transport.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	case "difficult":
	{
		...
		_crate_items 		= (18 + (round (random 3)));
	...
	default
	{
		...
		_crate_items 		= (30 + (round (random 6)));
...
_msgStart = ['#FFFF00',"A Beer Delivery Truck has been collapsed and the Prisoners are trying to protect the crash site. Kill the Prisoners and get the Beer and their loot!"];
...
if (_difficulty isEqualTo "hardcore") then {
												...
												_vehicle = ["CUP_B_HMMWV_DSHKM_GPK_ACR",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
												_msgWIN = ['#0080ff',format ["The Prisoners are Killed and the Beer has been Stolen, entry code of HMMWV is %1...",_pinCode]];
											} else
											{
												_vehicle = ["CUP_B_HMMWV_DSHKM_GPK_ACR",[(_pos select 0) -30, (_pos select 1) -30,0],[], 0, "CAN_COLLIDE"] call DMS_fnc_SpawnNonPersistentVehicle;
												_msgWIN = ['#0080ff',"The Prisoners are Killed and the Beer has been Stolen!"];
											};
...
_msgLOSE = ['#FF0000',"The Beer has been Drunken and the Guns are destroyed..."];

// Define mission name (for map marker and logging)
_missionName = "Beer and Guns";
