...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_weed_mission.sqf with invalid parameters: %1",_this];
};
...
_msgStart = ['#FFFF00',"Prisoners are making the best of it and setup a weed farm! Kill the Prisoners while they're high as a kite!"];
...
if (_difficulty isEqualTo "easy") then
{
	...
	_msgWIN = ['#0080ff',"The Stoned Prisoners are no more."];
}
else
{
	_pinCode = (1000 +(round (random 8999)));
	_vehicle = [_VehicleClass,[(_pos select 0) -20, (_pos select 1) -20, 0],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#0080ff',format ["The Stoned Prisoners are no more. Entry code of SUV is %1...",_pinCode]];
};
...
_msgLOSE = ['#FF0000',"The Prisoners Smoked all the Weed and went to Trader City to get some munchies!"];

// Define mission name (for map marker and logging)
_missionName = "Stoned Prisoners";
