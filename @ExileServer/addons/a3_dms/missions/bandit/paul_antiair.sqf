...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_antiair.sqf with invalid parameters: %1",_this];
};
...
_msgStart = ['#FFFF00',"Soldiers are setting up Anti Air to reduce the amount of Jets... Go stop them and steal their Tigiris!!"];
...
if (_difficulty isEqualTo "hardcore") then {
												...
												_vehicle = ["B_APC_Tracked_01_AA_F",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
												_msgWIN = ['#0080ff',format ["The soldiers are cleaned up! Entry code of MBT-52 Kuma is %1...",_pinCode]];
											} else
											{
												_vehicle = ["B_APC_Tracked_01_AA_F",[(_pos select 0) -30, (_pos select 1) -30,0],[], 0, "CAN_COLLIDE"] call DMS_fnc_SpawnNonPersistentVehicle;
												_msgWIN = ['#0080ff',"The soldiers are cleaned up and the MBT-52 Kuma is being stolen!"];
											};
...
_msgLOSE = ['#FF0000',"The soldiers accidently destroyed their Anti Air. Everyone died"];

// Define mission name (for map marker and logging)
_missionName = "Anti Air";
