...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_sandbags_mission.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	case "moderate":
	{
		...
		_crate_weapons 		= (12 + (round (random 1)));
...
_msgStart = ['#FFFF00',"Soldiers have setup a camp build with Sandbags. Kill them and try to steal their M1A2 Tusk!"];
...
if (_difficulty isEqualTo "easy") then
{
	...
	_msgWIN = ['#0080ff',"Convicts killed every last one of the soldiers and are trying to make a run with the M1A2 TUSK..."];
}
else
{
	_pinCode = (1000 +(round (random 8999)));
	_vehicle = [_VehicleClass,[(_pos select 0) -20, (_pos select 1) -20, 0],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#0080ff',format ["Convicts killed every last one of the soldiers and grabbed the loot! Entry code of M1A2 TUSK is %1...",_pinCode]];
};
...
_msgLOSE = ['#FF0000',"The Soldiers packed their stuff and moved on."];

// Define mission name (for map marker and logging)
_missionName = "Soldier's Base";
