...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_hmmwv.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	case "difficult":
	{
		...
		_crate_weapons 		= (17 + (round (random 0)));
		_crate_items 		= (4 + (round (random 0)));
...
_msgStart = ['#FFFF00',"An Armed HMMWV is being Repaired. Steal the Armed HMMWV!"];
...
if (_difficulty isEqualTo "easy") then {
												_vehicle = ["CUP_B_HMMWV_M1114_USMC",[(_pos select 0) -30, (_pos select 1) -30,0],[], 0, "CAN_COLLIDE"] call DMS_fnc_SpawnNonPersistentVehicle;
												_msgWIN = ['#0080ff',"The Soldiers have been Emiminated and the Armed HMMWV has been stolen!"];
												
											} else
											{
												_pinCode = (1000 +(round (random 8999)));
												_vehicle = ["CUP_B_HMMWV_M1114_USMC",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
												_msgWIN = ['#0080ff',format ["The Soldiers have been Emiminated and the Armed HMMWV has been stolen, entry code is %1...",_pinCode]];
											};
...
_msgLOSE = ['#FF0000',"The Soldiers got away with the HMMWV!"];

// Define mission name (for map marker and logging)
_missionName = "Soldiers Protecting Armed HMMWV";
