...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_church_mission.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	{
		_AICount = (5 + (round (random 2)));
		_crate_weapons 		= (12 + (round (random 2)));
		_crate_items 		= (15 + (round (random 4)));
	...
	default
	{
		...
		_crate_weapons 		= (20 + (round (random 2)));
		_crate_items 		= (20 + (round (random 4)));
		_crate_backpacks 	= (4 + (round (random 1)));
	};
};

_msgStart = ['#FFFF00',"A Church has been Captured by Prisoners. Retake the Church and Claim the Loot!"];
...
_missionObjs =
[
	_baseObjs,					// armed AI vehicle and static guns
...
_msgWIN = ['#0080ff',format ["The Church has been retaken! Entry code of Armed SUV is %1...",_pinCode]];

// Define Mission Lose message
_msgLOSE = ['#FF0000',"The Prisoners destroyed the Church..."];

// Define mission name (for map marker and logging)
_missionName = "Captured Church";
