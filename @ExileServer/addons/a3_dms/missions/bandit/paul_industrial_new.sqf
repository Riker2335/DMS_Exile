...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_industrial_new.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	{
		_AICount = (4 + (round (random 2)));
		_crate_weapons 		= (6 + (round (random 0)));
	...
	case "difficult":
	{
		...
		_crate_weapons 		= (8 + (round (random 0)));
...
_msgStart = ['#FFFF00',"A bunch of Prisoners have gathered around with expensive Building Materials. Eliminate them!"];


_crate_item_list	= ["Exile_Item_Handsaw","Exile_Item_ConcreteWallKit","Exile_Item_Flag","Exile_Item_Foolbox","Exile_Item_Grinder","Exile_Item_MetalBoard","Exile_Item_Hammer","Exile_Item_MetalPole","Exile_Item_Foolbox","Exile_Item_DuctTape","Exile_Item_Pliers","Exile_Item_MetalHedgehogKit","Exile_Item_Sand","Exile_Item_Shovel","Exile_Item_WaterBarrelKit"]; 
...
if (_difficulty isEqualTo "hardcore") then {
												...
												_msgWIN = ['#0080ff',format ["The Prisoners have been eliminated and the Building Supplies are stolen! Entry Code of Ural is",_pinCode]];
											} else
											{
...
_msgLOSE = ['#FF0000',"The Prisoners have packed their stuff up and moved on."];
