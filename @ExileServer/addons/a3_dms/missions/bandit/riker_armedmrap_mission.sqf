/*
	Armed Vehicle Mission variant with new difficulty selection system & random vehicle choice
	Vehicle not persistent on easy, 50/50 on moderate and 100% on difficult and hardcore
	based on work by Defent and eraser1, new mission content by Riker.
	easy/mod/difficult/hardcore - reworked by [CiC]red_ned http://cic-gaming.co.uk
*/

private ["_num", "_side", "_pos", "_OK", "_difficulty", "_extraParams", "_AICount", "_group", "_type", "_launcher", "_staticGuns", "_wreck", "_crate", "_crate1", "_vehicle", "_pinCode", "_class", "_veh", "_crate_loot_values", "_crate_loot_values1", "_missionAIUnits", "_missionObjs", "_msgStart", "_msgWIN", "_msgLOSE", "_missionName", "_markers", "_time", "_added", "_cleanup", "_baseObjs", "_crate_weapons", "_crate_weapon_list", "_crate_items", "_crate_item_list", "_crate_backpacks", "_PossibleDifficulty", "_PossibleEasyVehicleClass", "_PossibleHardVehicleClass", "_VehicleClass"];
...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION riker_armedmrap_mission.sqf with invalid parameters: %1",_this];
};
...
[
		...
		"CUP_O_GAZ_Vodnik_PK_RU",
		"CUP_O_BTR90_HQ_RU",				// BTR90 with machine guns
		"CUP_O_BTR90_HQ_RU",				// BTR90 with machine guns
		//Extra Chances for the high end vehicles with small chance for Grenade variant
		...
		"O_MRAP_02_hmg_F",					//Ifrit
		"CUP_O_BTR90_RU"					//BTR90 with cannon
];
...
switch (_difficulty) do
{
	case "easy":
	{
		_msgWIN = ['#0080ff',"Convicts killed everyone and made off with the armed vehicle"];
		_AICount = (10 + (round (random 4)));
		_crate_weapons 		= (1 + (round (random 2)));
		_crate_items 		= (3 + (round (random 4)));
		_crate_backpacks 	= (1 + (round (random 2)));
		//choose & spawn the vehicle
		_VehicleClass = selectRandom _PossibleEasyVehicleClass;
	...
	case "moderate":
	{
		...
		else
		{
			...
			_msgWIN = ['#0080ff',format ["Convicts killed everyone and made off with the vehicle, entry code %1...",_pinCode]];
		};
	};
	...
	case "difficult":
	{
		...
		//choose & spawn the vehicle
		_VehicleClass = selectRandom _PossibleHardVehicleClass;
		_vehicle = [_VehicleClass,[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	};
	...
	default
	{
		...
		_crate_items 		= (10 + (round (random 8)));
...
//_msgStart = ['#FFFF00',format["An Armed vehicle is parked at a small %1 base! Go kill them and steal it",_difficulty]];
_msgStart = ['#FFFF00',"Terrorists are protecting a crashed heli, kill them and steal their vehicle"];
...
/* no patrol vehicle, doesn't fit with the mission
// add vehicle patrol
_veh =
[
	[
...
_missionObjs =
[
	_staticGuns+_baseObjs,					// armed AI vehicle and static guns
...
_msgLOSE = ['#FF0000',"The terrorists secured the crashed heli and drove off."];

// Define mission name (for map marker and logging)
_missionName = "Armed MRAP Steal";
