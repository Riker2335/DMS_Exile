...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_fisher.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	case "difficult":
	{
		...
		_crate_weapons 		= (14 + (round (random 1)));
	...
	default
	{
		...
		_crate_weapons 		= (16 + (round (random 1)));
		_crate_items 		= (20 + (round (random 7)));
		_crate_backpacks 	= (4 + (round (random 1)));
	};
};

_msgStart = ['#FFFF00',"A Group of Fishers managed to Crash their Boat on Land..."];
...
if (_difficulty isEqualTo "easy") then
{
	...
	_msgWIN = ['#0080ff',"Convicts killed the Fishers"];
}
else
{
	_pinCode = (1000 +(round (random 8999)));
	_vehicle = [_VehicleClass,[(_pos select 0) -20, (_pos select 1) -20, 0],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#0080ff',format ["Convicts killed the Fishers, entry code %1...",_pinCode]];
};
...
_msgLOSE = ['#FF0000',"The Fishers pushed their Boat into the Water and Escaped!"];

// Define mission name (for map marker and logging)
_missionName = "Crashed Fishers";
