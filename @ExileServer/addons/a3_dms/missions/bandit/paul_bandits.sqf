...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_bandits.sqf with invalid parameters: %1",_this];
};
...
_msgStart = ['#FFFF00',"A group of Prisoners have crashed their C-192. Kill the Prisoners and get the Guns!"];
...
// [
// 	[
// 		[(_pos select 0) -60,(_pos select 1)+60,0]
// 	],
// 	_group,
// 	"assault",
// 	_difficulty,
...
if (_difficulty isEqualTo "hardcore") then
{
	...
	_vehicle = ["Exile_Car_SUV_Black",_pos getPos [30, random 360],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#0080ff',format ["The Prionsers are Killed and the Guns have been looted! Entry Code for SUV is %1...",_pinCode]];
} else
{
	_vehicle = ["Exile_Car_SUV_Black",_pos getPos [30, random 360]] call DMS_fnc_SpawnNonPersistentVehicle;
	_msgWIN = ['#0080ff',"The Prionsers are Killed and the Guns have been looted!"];
};
...
_msgLOSE = ['#FF0000',"The Prisoners managed to fix their C-192 and flew away with the guns..."];

// Define mission name (for map marker and logging)
_missionName = "Guns and Fun";
