...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_safe_mission.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	default
	{
		...
		_crate_items 		= (3 + (round (random 1)));
		_crate_backpacks 	= (10 + (round (random 1)));
	};
};

//_msgStart = ['#FFFF00',format["A field hospital is under attack! Go kill the %1 attackers",_difficulty]];
_msgStart = ['#FFFF00',"A Safe is being protected by bandits! Go kill the bandits, and get the safe!"];

_crate_item_list	= ["Exile_Item_Safekit"];
...
if (_difficulty isEqualTo "hardcore") then {
											...
											} else
											{
												...
												_msgWIN = ['#0080ff',"The bandits got away with theire Safe!"];
											};
...
_msgLOSE = ['#FF0000',"The attackers killed the bandits and stole everything!"];

// Define mission name (for map marker and logging)
_missionName = "Bandits Protecting Safe";
