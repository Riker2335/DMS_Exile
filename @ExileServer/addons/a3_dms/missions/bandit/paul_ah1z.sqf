...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_ah1z.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	{
		_AICount = (6 + (round (random 1)));
		_crate_weapons 		= (8 + (round (random 1)));
		_crate_items 		= (8 + (round (random 7)));
		_crate_backpacks 	= (4 + (round (random 1)));
	};
};

_msgStart = ['#FFFF00',"Soldiers are having a fight about an unarmed AH1Z. Kill the Soldiers while they're busy, and get the AH1Z!"];
...
else
{
	...
	_vehicle = [_VehicleClass,[(_pos select 0) -25, (_pos select 1) -25, 0],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
	_msgWIN = ['#0080ff',format ["The Fighting Soldiers have been Eliminated and Prisoners are taking off with the AH1Z, entry code %1...",_pinCode]];
};
...
_msgLOSE = ['#FF0000',"The Fight is over and the Soldiers flew away with the AH1Z!"];

// Define mission name (for map marker and logging)
_missionName = "AH1Z Steal";
