...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_bambi_slide.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	case "difficult":
	{
		...
		_crate_weapons 		= (12 + (round (random 0)));
		_crate_items 		= (12 + (round (random 0)));
		_crate_backpacks 	= (3 + (round (random 1)));
	};
...
_msgStart = ['#FFFF00',"Bambi's are caught playing on a slide. Stop them!"];

_crate_item_list	= ["Exile_Item_BushKit_Green","Exile_Item_Bandage","Exile_Item_Beer","Exile_Item_JunkMetal","Exile_Item_Foolbox","Exile_Item_CatFood","Exile_Item_CockONut","Exile_Item_DuctTape","Exile_Item_Handsaw"];
...
[
	...
	"bambi", 				// "random","assault","MG","sniper" or "unarmed" OR [_type,_launcher]
...
if (_difficulty isEqualTo "hardcore") then {
												...
												_msgWIN = ['#0080ff',format ["Convicts killed the bandits and stole the safe and the Ural, entry code is %1...",_pinCode]];
											} else
											{
												_vehicle = ["Exile_Car_Hatchback_Grey",[(_pos select 0) -30, (_pos select 1) -30,0],[], 0, "CAN_COLLIDE"] call DMS_fnc_SpawnNonPersistentVehicle;
												_msgWIN = ['#0080ff',"The Bambi's are stopped by a true hero!"];
											};
...
_msgLOSE = ['#FF0000',"The Bambi's stopped playing on the slide and moved on with their lives!"];

// Define mission name (for map marker and logging)
_missionName = "Bambi's Playing on Slide";
