/*
	Bank Robbery Mission designed to use resources from Crazy Mike's Exile Items mod. Original mission template by [CiC]red_ned
...
switch (_difficulty) do
{
	case "easy":
	{
		...
		_crate_weapons		= 2;
		_crate_items 		= (20 + (round (random 20)));
		_crate_backpacks 	= 2;
	};
	...
	case "moderate":
	{
		...
		_crate_weapons		= 3;
		_crate_items 		= (30 + (round (random 40)));
		_crate_backpacks 	= 3;
	};
	...
	case "difficult":
	{
		...
		_crate_weapons		= 4;
		_crate_items 		= (50 + (round (random 50)));
		_crate_backpacks 	= 3;
	};
	...
	default
	{
		_AICount = (10 + (round (random 4)));
		_crate_weapons		= 5;
		_crate_items 		= (80 + (round (random 70)));
		_crate_backpacks 	= 4;
	};
};
...
//_msgStart = ['#FFFF00',format["Bandits are robbing an armoured car. Stop them and get that cash!",_difficulty]];
_msgStart = ['#FFFF00',"Bandits are robbing a cash transport. Stop them and get the cash!"];
_crate_item_list	= ["Exitem_money"];
...
_missionObjs =
[
	_staticGuns+[_wreck]+[_veh],			// base objects, static guns & AI Vehicle
...
// Define Mission Win Messagee
_msgWIN = ['#0080ff',"Convicts killed the bandits and got the cash!"];

// Define Mission Lose message
_msgLOSE = ['#FF0000',"The Bandits cleaned out the cash truck and got away!"];

// Define mission name (for map marker and logging)
_missionName = "Cash Truck";
