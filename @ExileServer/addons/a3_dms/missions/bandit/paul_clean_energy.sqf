...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION paul_clean_energy.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	...
	case "difficult":
	{
		...
		_crate_weapons 		= 17;
		_crate_items 		= (12 + (round (random 8)));
		_crate_backpacks 	= 5;
	};
	...
	default
	{
		...
		_crate_weapons 		= 20;
		_crate_items 		= (15 + (round (random 10)));
		_crate_backpacks 	= 5;
	};
};
...
_msgStart = ['#FFFF00',"A Group of Volunteers have setup a Camp for Clean Energy. Let no one Escape!"];

// Define Mission Win message
_msgWIN = ['#0080ff',format ["No One Escaped from the Prisoners! Entry code %1...",_pinCode]];

// Define Mission Lose message
_msgLOSE = ['#FF0000',"The Volunteers got into a Fight and killed each other!"];

// Define mission name (for map marker and logging)
_missionName = "Clean Energy";
