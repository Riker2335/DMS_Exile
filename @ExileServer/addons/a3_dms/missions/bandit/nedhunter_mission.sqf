...
switch (_difficulty) do
{
	case "easy":
	{
		_msgWIN = ['#0080ff',"Convicts killed everyone and made off with the Hunter, entry code %1...",_pinCode"];
		_AICount = (4 + (round (random 4)));
		_crate_weapons 		= (2 + (round (random 2)));
		_crate_items 		= (4 + (round (random 4)));
		_crate_backpacks 	= (1 + (round (random 2)));
		_vehicle = [_VehicleClass,[(_pos select 0) -30, (_pos select 1) -30]] call DMS_fnc_SpawnPersistentVehicle;
	};
