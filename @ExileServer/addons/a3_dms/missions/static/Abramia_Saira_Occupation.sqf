...
_AIMaxReinforcements = (3 + (round (random 2)));		//AI reinforcement cap
_AItrigger = (1 + (round (random 1)));					//If AI numbers fall below this number then reinforce if any left from AIMax
_AIwave = (5 + (round (random 1)));						//Max amount of AI in reinforcement wave
_AIdelay = (10 + (round (random 1)));					//The delay between reinforcements
_crate_weapons0 	= (1 + (round (random 1)));			//Crate 0 weapons number (General weapons)
_crate_items0 		= (10 + (round (random 10)));		//Crate 0 items number
_crate_backpacks0 	= (3 + (round (random 1)));			//Crate 0 back packs number
...
_AICount = 24; 
_AIMaxReinforcements = (5 + (round (random 5)));
_AItrigger = (5 + (round (random 5)));
_AIwave = (5 + (round (random 3)));
_AIdelay = (55 + (round (random 120)));
_crate_weapons0 	= (4 + (round (random 3)));			// Crate 0 = General Items
...
_AIdelay = (55 + (round (random 120)));
_crate_weapons0 	= (5 + (round (random 5)));		// Crate 0 = General Items
_crate_items0 		= (20 + (round (random 10)));
_crate_backpacks0 	= (3 + (round (random 1)));
...
[
	[
		[5496.8008,1547.666,0],	
		[5524.498,1536.173,0],		
		[5446.4946,1450.2976,0],
		[5338.8301,1341.6051,0],
		[5335.1577,1493.0598,0]
	],
	_group,
	"assault",
	_difficulty,
...
// Define some randomised spawn locations for the reinforcements
...
[
	[5444.5142,1473.1189,0],
	[5402.0928,1392.6222,0],
	[5386.2769,1383.3187,0],
	[5516.1309,1478.3723,0],
	[5495.8257,1416.3729,0]
];
_crate4_position = selectRandom _crate4_position_list;

_crateClasses_and_Positions =
[
	[[5408.3789,1444.0726,0],"I_CargoNet_01_ammo_F"],
	[[5418.2969,1442.6417,0],"I_CargoNet_01_ammo_F"],
	[[5398.7549,1441.2881,0],"I_CargoNet_01_ammo_F"],
	[[5421.9126,1433.4307,0],"I_CargoNet_01_ammo_F"],
...
_msgStart = ['#FFFF00',format["Saria Armybase has been taken over by Militia! There are reports of a large loot cache....",_difficultyM]];

// Define Mission Win message
_msgWIN = ['#0080ff',"Convicts have successfully assaulted the Saria Armybase and secured the cache!"];

// Define Mission Lose message
_msgLOSE = ['#FF0000',"The Militia have left Saria Armybase, taking the cache with them..."];

// Define mission name (for map marker and logging)
_missionName = "Saria Militia";
